...
public class MaximumSegmentSumAfterRemovals {
    public static long[] maximumSegmentSum(int[] nums, int[] removeQueries) {
        ...
        // TreeMap to store to oits sum mapping
        ...
        // TreeMap to store all the possible sums of range we encounter while solving
        // queries, we are storing frequencies because multiple range can have same sum.
        ...
        for (int i = 0; i < n; i++) {
            ...
            // removing/ reducing sum from sums Map because we are splitting that range, so
            // it is no more valid
            int f = sums.get(sum);
            if (f == 1)
                sums.remove(sum);
            ...
            // spliting range and store back new ranges form along with its sum
            if (l == node && r != node) {
    ...
    public static void main(String[] args) {
        ...
        int[] removeQueries = new int[n];

        for (int i = 0; i < n; i++) {
            removeQueries[i] = scr.nextInt();
        }

        long[] ans = maximumSegmentSum(nums, removeQueries);
